...
function escapeRegex(value){return String(value??'').replace(/[.*+?^${}()|[\\]\\]/g,'\\function escapeRegex(value){return String(value??'').replace(/[.*+?^${}()|[\\]\\]/g,'\\function result(html){const e=document.getElementById('agentResult');if(e)e.innerHTML=html}')}
function highlightStory')}\nfunction highlightStory(text,rows){const story=String(text??'').trim(),phrases=[...new Set((Array.isArray(rows)?rows:[]).map(row=>String(row?.c??'').trim()).filter(Boolean))].sort((a,b)=>b.length-a.length);if(!story||!phrases.length)return esc(story);const pattern=phrases.map(escapeRegex).join('|');if(!pattern)return esc(story);const re=new RegExp('(?<![\\p{L}\\p{N}])(?:'+pattern+')(?![\\p{L}\\p{N}])','giu');let html='',last=0;for(const match of story.matchAll(re)){html+=esc(story.slice(last,match.index));html+='<mark class="story-collocation">'+esc(match[0])+'</mark>';last=match.index+match[0].length}html+=esc(story.slice(last));return html||esc(story)}
