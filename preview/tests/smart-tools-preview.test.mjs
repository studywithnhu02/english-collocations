...
test('suggestions render CEFR, existing-row warning, meanings and multi-source expansion',()=>{for(const token of ['suggestion-cefr','suggestion-exists','✓ Đã có trong bảng','getExternalSuggestions',"purpose:'suggestions'","purpose:'suggestion-meanings'","maxNewTokens:768",'slice(0,60)','Mỗi gợi ý đều có nghĩa tiếng Việt','x.v||x.meaningEn'])assert.ok(js.includes(token),token)});
