import {fastSuggestions,synonymsFor,ruleRefinement,refinementPrompt} from './smart-tools-core.mjs';
...
function showSuggestions(cell,values,label){
  const clean=[...new Map(values.map(value=>[String(value).trim().toLowerCase(),String(value).trim()])).values()].filter(Boolean).slice(0,5);
  if(!clean.length){closeSuggest();return}
  closeSuggest();suggestCell=cell;suggestValues=clean;suggestIndex=0;
  ...
  clean.forEach((value,index)=>{const b=document.createElement('button');b.type='button';b.className='suggestion-btn';b.textContent=value;b.setAttribute('role','option');b.setAttribute('aria-selected',String(index===0));b.addEventListener('mousedown',e=>e.preventDefault());b.addEventListener('click',()=>fill(cell,value));list.appendChild(b)});
...
function sanitizeAiSuggestions(text,value){
  ...
  const values=parseArray(text).map(x=>typeof x==='string'?x:String(x?.collocation??x?.phrase??'')).map(x=>x.trim()).filter(Boolean);
  const unique=[...new Map(values.map(v=>[v.toLowerCase(),v])).values()];
  return unique.filter(v=>{const low=v.toLowerCase();return low!==q&&low.startsWith(head)&&low.length<=90}).slice(0,5);
}
  ...
  timer=setTimeout(async()=>{
    try{
      const raw=await aiJson([
        {role:'system',content:'Return only a JSON array of 3-5 common English collocations beginning with the supplied headword. Use natural workplace English. No explanations, no numbering.'},
...
function handleFocus(e){
  ...
  const fast=fastSuggestions(value);
  if(fast.length)showSuggestions(cell,fast,'⚡ Gợi ý nhanh');
}
